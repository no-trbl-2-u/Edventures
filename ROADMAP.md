...
- [ ] Pick apex-vs-`www` once (0.2) and 301 the other; never serve both — apex is picked and canonical everywhere, but `www` still answers 200. Needs a zone Redirect Rule the API token cannot create (NEEDS_HUMAN_ATTENTION.md). Not urgent: canonicals consolidate on the apex regardless
- [x] Unique `<title>` per page, under ~60 chars, each ending `· Edventures Pet Sitting` — longest is 52. The home page no longer titles itself with the bare brand name, which told a searcher nothing they had not already typed
- [x] Unique meta description per page, 140–160 chars, written as ad copy rather than a summary — it's the click-through pitch. All 8 unique; the 7 indexable ones are 140–160 (404 exempt, it is not a landing page)
- [x] One `h1` per page containing the real subject, not the logo — exactly one per page. The home page's was the brand and tagline; it now names the service and the city, so a screen reader opens on what is sold rather than on a name that means nothing yet
- [x] Descriptive `alt` on every image (already sourced from `photos.json`) — also feeds Google Images, which matters more than expected for pet businesses. Audited: 33 described, 21 decorative with an explicit empty `alt`, 0 missing
- [x] Verify no page is orphaned — every page reachable from the nav or footer. All 7 indexable pages are linked from the home page; only `/404` is unlinked, which is correct
